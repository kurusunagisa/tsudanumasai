 ...
 */ p5.prototype.registerMethod("init", function p5PlayInit() {
    ...
    if (typeof window.planck == "undefined") throw new Error("planck.js must be loaded before p5.play");
    const pl = planck;
    ...
    const fixRound = (val1)=>Math.abs(val1 - Math.round(val1)) <= pl.Settings.linearSlop ? Math.round(val1) : val1;
    const isArrowFunction = (fn)=>!/^(?:(?:\/\*[^(?:\*\/)]*\*\/\s*)|(?:\/\/[^\r\n]*))*\s*(?:(?:(?:async\s(?:(?:\/\*[^(?:\*\/)]*\*\/\s*)|(?:\/\/[^\r\n]*))*\s*)?function|class)(?:\s|(?:(?:\/\*[^(?:\*\/)]*\*\/\s*)|(?:\/\/[^\r\n]*))*)|(?:[_$\w][\w0-9_$]*\s*(?:\/\*[^(?:\*\/)]*\*\/\s*)*\s*\()|(?:\[\s*(?:\/\*[^(?:\*\/)]*\*\/\s*)*\s*(?:(?:['][^']+['])|(?:["][^"]+["]))\s*(?:\/\*[^(?:\*\/)]*\*\/\s*)*\s*\]\())/.test(fn.toString());
    let spriteProps = [
        "bounciness",
        "collider",
        "color",
	 ...
	 */ class Sprite {
        constructor(x, y, w, h, collider){
            ...
            if (args.length == 1 && typeof args[0] == "number") throw new Error("Invalid input parameters for the Sprite constructor. You can't only have one number! Specify both the x and y position of the sprite");
            x = args[0];
            y = args[1];
            w = args[2];
            h = args[3];
            ...
            if (Array.isArray(w) || typeof h == "string") {
                ...
                if (typeof w != "number" && Array.isArray(w[0])) this.originMode = "start";
                if (h !== undefined) {
                    let abr = h.slice(0, 2);
                    // h is a collider type
                    if (abr == "dy" || abr == "st" || abr == "ki" || abr == "no") collider = h;
                    else {
                        // h is the name of a regular polygon
                        if (h == "circle") ;
                        else if (h == "triangle") w = [
                            w,
                            -120,
                            3
                        ];
                        ...
                        else if (h == "hendecagon") w = [
                            w,
                            -32.7272727273,
                            11
                        ];
                        else if (h == "dodecagon") w = [
                            w,
                            -30,
                            12
                        ];
            ...
            if (collider !== undefined) {
                ...
                let abr1 = collider.slice(0, 2);
                if (abr1 == "dy") collider = "dynamic";
                if (abr1 == "st") collider = "static";
                if (abr1 == "ki") collider = "kinematic";
                if (abr1 == "no") collider = "none";
            }
            /**
            ...
            if (group) {
                group.push(this);
                if (!ani) for(let _ani in group.animations){
                    ani = _ani;
                    break;
                }
            } else group = this.p.allSprites;
			 ...
			 */ this.vel = {
                get x () {
                    let val1;
                    if (!_this1.body) val1 = _this1._vel.x;
                    else val1 = _this1.body.getLinearVelocity().x;
                    return fixRound(val1 / _this1.tileSize);
                },
                set x (val){
                    val *= _this1.tileSize;
                    if (_this1.body) _this1.body.setLinearVelocity(new pl.Vec2(val, _this1.body.getLinearVelocity().y));
                    else _this1._vel.x = val;
                },
                get y () {
                    let val2;
                    if (!_this1.body) val2 = _this1._vel.y;
                    else val2 = _this1.body.getLinearVelocity().y;
                    return fixRound(val2 / _this1.tileSize);
                },
                set y (val){
                    val *= _this1.tileSize;
                    if (_this1.body) _this1.body.setLinearVelocity(new pl.Vec2(_this1.body.getLinearVelocity().x, val));
                    else _this1._vel.y = val;
                }
            };
            /**
            ...
            this.mirror = {
                get x () {
                    return _this1._mirror.x < 0;
                },
                set x (val){
                    _this1._mirror.x = val ? -1 : 1;
                },
                get y () {
                    return _this1._mirror.y < 0;
                },
                set y (val){
                    _this1._mirror.y = val ? -1 : 1;
                }
            };
            if (ani) {
                if (ani instanceof p5.Image) this.addAni(ani);
                else if (typeof ani == "string") this._changeAni(ani);
                else this._animation = ani.clone();
                let ts = this.tileSize;
                if (!w && this.ani.w != 1) {
                    w = this.ani.w / ts;
                    if (this.shape != "circle" && this.ani.h > 1) h = this.ani.h / ts;
                }
            }
            this.layer = group.layer;
            this.layer ??= this.p.allSprites.maxDepth() + 1;
            collider ??= group.collider;
            if (!collider || typeof collider != "string") collider = "dynamic";
            x ??= group.x;
            if (x === undefined) {
            ...
            for (let prop of spriteProps){
                ...
                let val3 = group[prop];
                if (val3 === undefined) continue;
                if (typeof val3 == "function") val3 = val3(group.length - 1);
                if (typeof val3 == "object") this[prop] = Object.assign({}, val3);
                else this[prop] = val3;
            }
            ...
            for (let g of this.groups){
                ...
                for (let prop1 of props){
                    if (!isNaN(prop1) || prop1[0] == "_") continue;
                    traits[prop1] = null;
                }
                // delete these traits
                let deletes = [
                ...
                for (let d of deletes)delete traits[d];
                for(let prop2 in traits){
                    let val4 = g[prop2];
                    if (val4 === undefined) continue;
                    if (typeof val4 == "function") {
                        if (isArrowFunction(val4)) val4 = val4();
                    }
                    if (typeof val4 == "object") this[prop2] = Object.assign({}, val4);
                    else this[prop2] = val4;
                }
            }
            /**
            ...
            for (let g1 of this.groups)if (g1._hasOverlaps) {
		 ...
		 */ addCollider(offsetX, offsetY, w, h) {
            ...
            // when in a tile grid
            if (shape == "box" || shape == "circle") dimensions = scaleTo({
                x: w - 0.08,
                y: h - 0.08
            }, this.tileSize);
            let s;
            if (shape == "box") s = pl.Box(dimensions.x / 2, dimensions.y / 2, scaleTo({
                x: offsetX,
                y: offsetY
            }, this.tileSize), 0);
                ...
                if (Math.round(vert.x * 1e6) / 1e6 == 0 && Math.round(vert.y * 1e6) / 1e6 == 0 && vecs.length - 1 <= pl.Settings.maxPolygonVertices && this._shape != "chain") shape = "polygon";
                else shape = "chain";
                w = max.x - min.x;
                this._hw = w * 0.5;
                h = max.y - min.y;
                this._hh = h * 0.5;
                if (this.originMode == "center") for(let i2 = 0; i2 < vecs.length; i2++){
                    let vec = vecs[i2];
                    vecs[i2] = new pl.Vec2((vec.x - this._hw - min.x) * this.tileSize / plScale, (vec.y - this._hh - min.y) * this.tileSize / plScale);
                }
                else // origin is start
                for(let i3 = 0; i3 < vecs.length; i3++){
                    let vec1 = vecs[i3];
                    vecs[i3] = new pl.Vec2(vec1.x * this.tileSize / plScale, vec1.y * this.tileSize / plScale);
                }
                if (shape == "polygon") {
                    if (this._isConvexPoly(vecs.slice(0, -1))) s = pl.Polygon(vecs);
                    else shape = "chain";
                }
                if (shape == "chain") {
            ...
            if (!this._shape) this._shape = shape;
            else if (this.fixture.getNext()) this._shape = "combo";
        ...
        _removeFixtures(isSensor) {
            let prevFxt;
            for(let fxt = this.fixtureList; fxt; fxt = fxt.getNext())if (fxt.m_isSensor == isSensor) {
                let _fxt = fxt.m_next;
                fxt.destroyProxies(this.p.world.m_broadPhase);
                if (!prevFxt) this.body.m_fixtureList = _fxt;
                else prevFxt.m_next = _fxt;
            } else prevFxt = fxt;
        }
        /**
		 ...
		 */ _cloneBodyProps() {
            ...
            for (let prop of props)body[prop] = this[prop];
            return body;
        }
        // get aabb() {
        // 	return getAABB(this);
        ...
        set allowSleeping(val1) {
            this.body.setSleepingAllowed(val1);
        }
        /**
        ...
        set bounciness(val1) {
            for(let fxt = this.fixtureList; fxt; fxt = fxt.getNext())fxt.setRestitution(val1);
        }
        /**
        ...
        _reset() {
            ...
            if (this._collider != "none") {
                if (v) this.addCollider(0, 0, v);
                else this.addCollider();
                if (this._hasOverlaps) this._createSensors();
                for(let prop in bodyProps)this[prop] = bodyProps[prop];
            }
        }
        /**
        ...
        set color(val1) {
            if (val1 instanceof p5.Color) this._color = val1;
            else if (typeof val1 != "object") this._color = this.p.color(val1);
            else this._color = this.p.color(...val1.levels);
        }
        get shapeColor() {
            return this._color;
        }
        ...
        set density(val1) {
            for(let fxt = this.fixtureList; fxt; fxt = fxt.getNext())fxt.setDensity(val1);
        }
        /**
		 * Use .layer instead.
		 *
		 ...
		 * @type {Number}
		 */ get direction() {
            if (this.body && (this.vel.x !== 0 || this.vel.y !== 0)) return this.p.atan2(this.vel.y, this.vel.x);
            if (!this._direction) return this.rotation;
            return this._direction;
        }
        set direction(val1) {
            this._direction = val1;
            let speed = this.speed;
            this.vel.x = this.p.cos(val1) * speed;
            this.vel.y = this.p.sin(val1) * speed;
        }
        /**
        ...
        set drag(val1) {
            if (this.body) this.body.setLinearDamping(val1);
        }
        /**
        ...
        set rotationLock(val1) {
            this.body.setFixedRotation(val1);
        }
        /**
        ...
        set friction(val1) {
            for(let fxt = this.fixtureList; fxt; fxt = fxt.getNext())fxt.setFriction(val1);
        }
        /**
		 * Use .static instead.
		 *
        ...
        set isSuperFast(val1) {
            this.body.setBullet(val1);
        }
        // get joint() {
        ...
        set kinematic(val1) {
            if (val1) this.collider = "kinematic";
        }
        /**
		 ...
		 */ get rotation() {
            ...
            if (this.p._angleMode === p5.prototype.DEGREES) return p5.prototype.degrees(this.body.getAngle());
            return this.body.getAngle();
        }
        set rotation(val1) {
            if (this.body) {
                if (this.p._angleMode === p5.prototype.DEGREES) this.body.setAngle(p5.prototype.radians(val1));
                else this.body.setAngle(val1);
            } else this._angle = val1;
        }
        /**
        ...
        set rotationDrag(val1) {
            this.body.setAngularDamping(val1);
        }
        /**
        ...
        set rotationSpeed(val1) {
            if (this.body) this.body.setAngularVelocity(val1);
            else this._rotationSpeed = val1;
        }
        /**
        ...
        set scale(val1) {
            ...
            this._resizeCollider(val1);
            this._scale = val1;
            if (this.ani) {
                this.ani.scale.x = val1;
                this.ani.scale.y = val1;
            }
        }
        /**
        ...
        set sleeping(val1) {
            return this.body.setAwake(!val1);
        }
        /**
		 * The sprite's speed.
		 *
		 ...
		 */ set speed(val1) {
            ...
            this.vel.x = this.p.cos(angle) * val1;
            this.vel.y = this.p.sin(angle) * val1;
        }
        /**
		 ...
		 */ set torque(val1) {
            this.body.applyTorque(val1, true);
        }
        /**
        ...
        set x(val1) {
            if (this.body) {
                let pos = new pl.Vec2(val1 * this.tileSize / plScale, this.body.getPosition().y);
        ...
        set y(val1) {
            if (this.body) {
                let pos = new pl.Vec2(this.body.getPosition().x, val1 * this.tileSize / plScale);
                this.body.setPosition(pos);
            }
            this._pos.y = val1;
        }
		 ...
		 * @type {Object}
		 */ set pos(val1) {
            let pos = new pl.Vec2(val1.x * this.tileSize / plScale, val1.y * this.tileSize / plScale);
        ...
        set w(val1) {
            if (val1 < 0) val1 = 0.01;
            if (val1 == this._w) return;
            let scale = val1 / this._w;
            this._w = val1;
            this._hw = val1 * 0.5;
        ...
        set h(val1) {
            ...
            if (val1 == this._h) return;
            let scale = val1 / this._h;
            this._h = val1;
            this._hh = val1 * 0.5;
            this._resizeCollider({
                y: scale
        ...
        _resizeCollider(scale) {
            ...
            if (this.shape == "circle") {
                ...
                let sh = fxt.m_shape;
                sh.m_radius *= scale.x;
            } else // let bodyProps = this._cloneBodyProps();
            // this.removeColliders();
            // this.addCollider();
            // for (let prop in bodyProps) {
            // 	this[prop] = bodyProps[prop];
            // }
            for(let fxt1 = this.fixtureList; fxt1; fxt1 = fxt1.getNext()){
                if (fxt1.m_isSensor) continue;
                let sh1 = fxt1.m_shape;
                for (let vert of sh1.m_vertices){
                    vert.x *= scale.x;
                    vert.y *= scale.y;
                }
            }
		 ...
		 */ _update() {
            ...
            for(let prop in this.mouse)if (this.mouse[prop] == -1) this.mouse[prop] = 0;
            let a = this;
            for(let event in eventTypes)for (let entry of this[event]){
                let contactType;
                let b = entry[0];
                let f = entry[1] + 1;
                this[event].set(b, f);
                if (f == 0) {
                    this[event].delete(b);
                    continue;
                } else if (f == -1) contactType = eventTypes[event][2];
                else if (f == 1) contactType = eventTypes[event][0];
                else contactType = eventTypes[event][1];
		 ...
		 * @private
		 */ _draw() {
            if (this.animation && !this.debug) this.animation.draw(0, 0, undefined, this._scale, this._scale);
            else if (this.fixture != null) {
                if (this._shape == "chain") this.p.stroke(this.color);
                for(let fxt = this.fixtureList; fxt; fxt = fxt.getNext())this._drawFixture(fxt);
            } else {
                this.p.stroke(120);
                if (this._shape == "box") this.p.rect(0, 0, this.w, this.h);
                else if (this._shape == "circle") this.p.circle(0, 0, this.d);
            }
            if (this.text) {
		 ...
		 */ _display() {
            ...
            if (this.shape != "chain" && this.p.camera.active && (x + this.w < this.p.camera.bound.min.x || x - this.w > this.p.camera.bound.max.x || y + this.h < this.p.camera.bound.min.y || y - this.h > this.p.camera.bound.max.y)) return;
            x = fixRound(x);
		 ...
		 */ _drawFixture(fxt) {
            ...
            if (sh.m_type == "polygon" || sh.m_type == "chain") {
                ...
                let v = sh.m_vertices;
                this.p.beginShape();
                for(let i = 0; i < v.length; i++)this.p.vertex(v[i].x * plScale, v[i].y * plScale);
            ...
            } else if (sh.m_type == "edge") this.p.line(sh.m_vertex1.x * plScale, sh.m_vertex1.y * plScale, sh.m_vertex2.x * plScale, sh.m_vertex2.y * plScale);
        }
        /**
		 ...
		 */ moveTowards(x, y, tracking) {
            ...
            if (x !== undefined && x !== null) // vec.x = (destX - this.x) * tracking * this.tileSize * this.mass;
            this.vel.x = (x - this.x) * tracking * this.tileSize;
            if (y !== undefined && y !== null) // vec.y = (destY - this.y) * tracking * this.tileSize * this.mass;
		 ...
		 */ move(destX, destY, speed) {
            ...
            if (typeof destX == "string") {
                ...
                if (direction == "up" || direction.slice(0, 2) == "up") this.dest.y = Math.round(this.y - 1);
                if (direction == "down" || direction.slice(0, 4) == "down") this.dest.y = Math.round(this.y + 1);
                if (direction == "left" || direction.includes("Left")) this.dest.x = Math.round(this.x - 1);
                if (direction == "right" || direction.includes("Right")) this.dest.x = Math.round(this.x + 1);
                destX = destY = false;
                if (/(up|down)/.test(direction)) destY = true;
                if (/(left|right)/.test(direction)) destX = true;
        ...
        snap(o, dist) {
            ...
            dist ??= 1;
            if (Math.abs(this.x) % 1 >= dist || Math.abs(this.y) % 1 >= dist) return;
            this.vel.x = 0;
            this.vel.y = 0;
		 ...
		 * @param {Number} speed
		 */ rotate(angle, speed) {
            if (!angle) throw new Error("angle must be a number greater or less than zero");
            let ang = this.rotation + angle;
            let mod = ang - this.rotation > 0 ? 1 : -1;
            this.rotationSpeed = speed * mod;
            return (async ()=>{
                let cw = ang > this.rotation;
                while(cw && this.rotation < ang || !cw && this.rotation > ang)await p5.prototype.delay();
		 ...
		 */ async changeAni(...anis) {
            if (anis.length == 1 && Array.isArray(anis[0])) anis = anis[0];
            let count = ++this._aniChanged;
            for(let i = 0; i < anis.length; i++){
                if (typeof anis[i] == "string") anis[i] = {
                    name: anis[i]
                };
                let ani = anis[i];
                if (ani.name[0] == "!") {
            ...
            for(let i1 = 0; i1 < anis.length; i1++){
                let ani1 = anis[i1];
                if (ani1.name == "*") {
                    if (count == this._aniChanged) i1 = 0;
                    continue;
                }
                let { name , start , end  } = ani1;
		 ...
		 */ _changeAni(label) {
            ...
            if (!ani) for (let g of this.groups){
            ...
            if (this.autoResetAnimations || this.autoResetAnimations !== false && this.p.world.autoResetAnimations) this.animation.frame = 0;
        }
        /**
		 ...
		 */ remove() {
            ...
            while(this.groups.length > 0)this.groups[0].remove(this);
        }
        /**
        ...
        _ensureCollide(target, callback) {
            if (!(target instanceof Sprite) && !(target instanceof Group)) throw new Error("collide target must be a sprite or a group");
        }
        /**
		 * @method collide
		 ...
		 */ colliding(target, callback) {
            ...
            let val1 = this._collisions.get(target);
        ...
        _ensureOverlap(target, callback) {
            ...
            if (target instanceof Sprite) {
                ...
                if (!target._hasOverlaps) {
                    for (let s of target)if (!s._hasOverlaps) s._createSensors();
		 ...
		 */ overlapping(target, callback) {
            ...
            let val1 = this._overlappers.get(target);
            return val1 > 0 ? val1 : 0;
        }
        /**
		 * Returns true on the first frame that the sprite no longer overlaps
	 ...
	 */ class SpriteAnimation extends Array {
        constructor(){
            ...
            if (args.length == 2 && typeof args[0] == "string" && (typeof args[1] == "string" || typeof args[1] == "number")) {
                ...
                if (from.slice(-4) != ".png" || to && to.slice(-4) != ".png") throw new Error("SpriteAnimation error: you need to use .png files (filename " + from + ")");
                let digits1 = 0;
                let digits2 = 0;
                ...
                if (to) for(let i1 = to.length - 5; i1 >= 0; i1--){
                    if (!isNaN(to.charAt(i1))) digits2++;
                    else break;
                }
                ...
                if (args[0] instanceof p5.Image || typeof args[0] == "string") {
                    if (args.length >= 3) throw new Error("SpriteAnimation error: the name of animation should go first");
                    sheet = args[0];
                    atlas = args[1];
                } else atlas = args[0];
                let _this1 = this;
                ...
                function _generateSheetFrames() {
                    ...
                    let { w , h , width , height , size , pos , line , x , y , frames: frames1 , delay , rotation  } = atlas;
                    if (delay) _this1.frameDelay = delay;
                    if (rotation) _this1.rotation = rotation;
                    w ??= width;
                    h ??= height;
                    let tileSize;
                    if (parent) {
                    ...
                    if (typeof size == "number") w = h = size;
                    else if (size) {
                        w = size[0];
                        h = size[1];
                    }
                    ...
                    y *= tileSize;
                    if (!w || !h) {
                        if (tileSize) w = h = 1 * tileSize;
                        else if (_this1.spriteSheet.width < _this1.spriteSheet.height) w = h = _this1.spriteSheet.width;
                        else w = h = _this1.spriteSheet.height;
                    } else {
                        w *= tileSize;
                        h *= tileSize;
                    }
                    let frameCount = frames1 || 1;
                    // add all the frames in the animation to the frames array
                    for(let i1 = 0; i1 < frameCount; i1++){
                        _this1.push({
                            x,
                            y,
                            w,
                        ...
                        x += w;
                        if (x >= _this1.spriteSheet.width) {
                            x = 0;
                            y += h;
                            if (y >= _this1.spriteSheet.height) y = 0;
                        }
                    }
                }
            } else {
                // list of images
                for(let i4 = 0; i4 < args.length; i4++)if (args[i4] instanceof p5.Image) this.push(args[i4]);
                else this.push(this.p.loadImage(args[i4]));
            }
        }
        clone() {
            let ani = new SpriteAnimation();
            ani.spriteSheet = this.spriteSheet;
            for(let i = 0; i < this.length; i++)ani.push(this[i]);
		 ...
		 */ draw(x, y, r, sx, sy) {
            ...
            if (img !== undefined) {
                if (this.spriteSheet) {
                    let { x: x1 , y: y1 , w , h  } = img; // image info
                    this.p.image(this.spriteSheet, this.offset.x, this.offset.y, w, h, x1, y1, w, h);
                } else this.p.image(img, this.offset.x, this.offset.y);
            } else this.p.print("Warning undefined frame " + this.frame);
            this.p.pop();
        }
        /**
		 * @private
		 */ update() {
            ...
            if (this.playing && this.cycles % this.frameDelay === 0) {
                ...
                if (this.targetFrame > this.frame && this.targetFrame !== -1) this.frame++;
                else if (this.targetFrame < this.frame && this.targetFrame !== -1) this.frame--;
                else if (this.targetFrame === this.frame && this.targetFrame !== -1) this.playing = false;
                else if (this.looping) {
                    //advance frame
                    //if next frame is too high
                    if (this.frame >= this.lastFrame) this.frame = 0;
                    else this.frame++;
                } else //if next frame is too high
		 ...
		 */ goToFrame(toFrame) {
            if (toFrame < 0 || toFrame >= this.length) return;
            ...
            if (this.targetFrame !== this.frame) this.playing = true;
        }
        /**
		 * Use .frame instead.
		 *
        ...
        get width() {
            if (this[this.frame] instanceof p5.Image) return this[this.frame].width;
            else if (this[this.frame]) return this[this.frame].w;
            return 1;
        }
        get h() {
            return this.height;
        ...
        get height() {
            if (this[this.frame] instanceof p5.Image) return this[this.frame].height;
            else if (this[this.frame]) return this[this.frame].h;
            return 1;
        }
        get frames() {
            let frames = [];
            for(let i = 0; i < this.length; i++)frames.push(this[i]);
            return frames;
        }
        get images() {
            return this.frames;
	 ...
	 */ class Group extends Array {
        constructor(...args){
            ...
            for(let state in this.mouse)this.mouse[state] = function(inp) {
                for (let s of _this1){
            ...
            for (let prop of props)Object.defineProperty(this, prop, {
                get () {
                    let val1 = _this1["_" + prop];
                    let i = _this1.length - 1;
                    if (val1 === undefined && this.p.world && !_this1._isAllSpritesGroup) {
                        let parent = this.p.world.groups[_this1.parent];
                        if (parent) {
                            val1 = parent[prop];
                            i = parent.length - 1;
                        }
                ...
                set (val1) {
                    ...
                    for(let i = 0; i < _this1.length; i++){
                        let s = _this1[i];
                        let v = val1;
                        if (typeof val1 == "function") v = val1(i);
                        s[prop] = v;
                    }
                }
            });
            ...
            for(let objProp in objProps)for (let prop1 of objProps[objProp])Object.defineProperty(this[objProp], prop1, {
                get () {
                    let val1 = _this1[objProp]["_" + prop1];
                    let i = _this1.length - 1;
                    if (val1 === undefined && _this1.p.world && !_this1._isAllSpritesGroup) {
                        let parent = _this1.p.world.groups[_this1.parent];
                        if (parent) {
                            val1 = parent[objProp][prop1];
                            i = parent.length - 1;
                        }
                    }
                    return val1;
        ...
        set amount(val1) {
            let diff = val1 - this.length;
            let shouldAdd = diff > 0;
            diff = Math.abs(diff);
            for(let i = 0; i < diff; i++)if (shouldAdd) this.add(new this.Sprite());
        ...
        resetDistancesFromCentroid() {
            for (let s of this)s.distCentroid = {
        ...
        snap(o, dist) {
            dist ??= 1;
            for (let s of this)s.snap(o, dist);
        }
        _ensureCollide(target, callback) {
            if (!(target instanceof Sprite) && !(target instanceof Group)) throw new Error("collide target must be a sprite or a group");
        }
        ...
        _ensureOverlap(target, callback) {
            ...
            if (!this._hasOverlaps) {
                for (let s of this)if (!s._hasOverlaps) s._createSensors();
                this._hasOverlaps = true;
            }
            if (target instanceof Sprite) {
                if (!target._hasOverlaps) target._createSensors();
                ...
                if (!target._hasOverlaps) {
                    for (let s1 of target)if (!s1._hasOverlaps) s1._createSensors();
                    target._hasOverlaps = true;
                }
            }
            this._overlap[target] = true;
		 ...
		 */ push(s) {
            if (!(s instanceof Sprite)) throw new Error("you can only add sprites to a group");
            if (this.indexOf(s) == -1) {
                super.push(s);
		 ...
		 */ cull(top, bottom, left, right, cb) {
            ...
            if (isNaN(top) || isNaN(bottom) || isNaN(left) || isNaN(right)) throw new TypeError("The culling boundary must be defined with numbers");
            if (cb && typeof cb != "function") throw new TypeError("The callback to group.cull must be a function");
            ...
            for (let s of this)if (s.x < minX || s.y < minY || s.x > maxX || s.y > maxY) {
                if (cb) cb(s);
                else s.remove();
            }
        }
		 ...
		 */ remove(item) {
            ...
            if (typeof item == "number") idx = item;
                ...
                for(let i = this.length - 1; i >= 0; i--)if (this[i] === item) {
                    idx = i;
                    break;
                }
            }
		 ...
		 */ maxDepth() {
            ...
            for (let s of this)if (s.layer > max) max = s.layer;
            return max;
        }
        /**
	 ...
	 */ Sprite.prototype.addAnis = Group.prototype.addAnis = Sprite.prototype.addAnimations = Group.prototype.addAnimations = Sprite.prototype.addImages = Group.prototype.addImages = Sprite.prototype.addImgs = Group.prototype.addImgs = function() {
        let args = arguments;
        let atlases;
        if (args.length == 1) atlases = args[0];
        else {
	 ...
	 */ class World extends pl.World {
        constructor(){
            ...
            this.offset = {
                get x () {
                    return -_this1._offset.x;
                },
                /**
				 * @property offset.x
				 */ set x (val){
                    _this1._offset.x = val;
                    _this1.resize();
                },
                get y () {
                    return -_this1._offset.y;
                },
                /**
				 * @property offset.y
				 */ set y (val){
                    _this1._offset.y = val;
                    _this1.resize();
                }
            };
            this.resize();
			 ...
			 */ this.gravity = {
                get x () {
                    return _this1.m_gravity.x;
                },
                set x (val){
                    _this1.m_gravity.x = _this1.p.round(val || 0);
                },
                get y () {
                    return _this1.m_gravity.y;
                },
                set y (val){
                    _this1.m_gravity.y = _this1.p.round(val || 0);
                }
            };
        }
        resize(w, h) {
        ...
        _beginContact(contact) {
            ...
            for (let g1 of a.groups){
                g1[t].set(b, g1[t].get(b) || 0);
                b[t].set(g1, b[t].get(g1) || 0);
                for (let g2 of b.groups){
                    g1[t].set(g2, g1[t].get(g2) || 0);
                    g2[t].set(g1, g2[t].get(g1) || 0);
                }
            }
        }
        _endContact(contact) {
            ...
            for (let g of b.groups){
                let inContact = false;
                for (let s of g)if (s[contactType].get(a) >= 0) {
                    inContact = true;
                    break;
                }
                if (!inContact) {
            ...
            for (let g1 of a.groups){
                let inContact1 = false;
                for (let s1 of g1)if (s1[contactType].get(b) >= 0) {
                    inContact1 = true;
                    break;
                }
                if (!inContact1) {
                    g1[contactType].set(b, -2);
                    b[contactType].set(g1, -2);
                    for (let g2 of b.groups){
                        g1[contactType].set(g2, -2);
                        g2[contactType].set(g1, -2);
                    }
                }
            }
        }
        ...
        set autoCull(val1) {
            this.p.allSprites.autoCull = val1;
        }
        get allowSleeping() {
            return this.getAllowSleeping();
        }
        set allowSleeping(val1) {
            this.setAllowSleeping(val1);
        }
    }
    /**
	 ...
	 */ function _findContactCB(type, s0, s1) {
        ...
        if (s1IsSprite) for (let g1 of s1.groups){
            cb = s0[type][g1];
            if (cb) return cb;
        }
        if (s0 instanceof Sprite) for (let g0 of s0.groups){
            cb = g0[type][s1];
            if (cb) return cb;
            if (s1IsSprite) for (let g11 of s1.groups){
                cb = g0[type][g11];
                if (cb) return cb;
            }
        }
	 ...
	 */ class Tiles {
        constructor(tiles, x, y, w, h){
            ...
            w ??= 1;
            h ??= 1;
            for(let row = 0; row < tiles.length; row++)for(let col = 0; col < tiles[row].length; col++){
                let t = tiles[row][col];
                if (t == " " || t == ".") continue;
                let ani, g;
                ...
                let wasFound = false;
                for (g of pInst.world.groups)if (g.tile == t) {
                    wasFound = true;
                    break;
                }
                if (wasFound) {
                ...
                let s;
                for (s of pInst.allSprites)if (s.tile == t) {
                    wasFound = true;
                    break;
                }
                if (wasFound) {
	 ...
	 */ p5.prototype.updateSprites = function(timeStep, velocityIterations, positionIterations) {
        ...
        for (let s1 of this.allSprites)s1.update();
	 ...
	 */ p5.prototype.getSpritesAt = function(x, y, group, cameraActiveWhenDrawn) {
        ...
        pInst.world.queryAABB(aabb, (fxt)=>{
            if (!fxt.getBody().isStatic()) {
                if (fxt.getShape().testPoint(fxt.getBody().getTransform(), convertedPoint)) fxts.push(fxt);
            }
            return true;
        });
        group ??= pInst.allSprites;
        let sprites = [];
        if (fxts.length > 0) for (let s of group){
            if (!s.body) continue;
	 ...
	 */ p5.prototype.colorPal = (c, palette)=>{
        ...
        if (typeof palette == "number") palette = pInst.world.palettes[palette];
        palette ??= pInst.world.palettes[0];
        let clr;
        if (palette) clr = palette[c];
        // if transparent
        if (clr === "" || c === "." || c === " ") return pInst.color(0, 0, 0, 0);
	 ...
	 */ p5.prototype.spriteArt = (txt, scale, palette)=>{
        ...
        let w = 0;
        for (let line of lines)if (line.length > w) w = line.length;
        let h = lines.length;
	 ...
	 */ p5.prototype.delay = (millisecond)=>{
        ...
        if (!millisecond) return new Promise(requestAnimationFrame);
        else // else it wraps setTimeout in a Promise
        return new Promise((resolve)=>{
	 ...
	 */ this.createCanvas = function() {
        ...
        this.canvas.addEventListener("keydown", function(e) {
            if (e.key == " " || e.key == "/" || e.key == "ArrowUp" || e.key == "ArrowDown" || e.key == "ArrowLeft" || e.key == "ArrowRight") e.preventDefault();
        });
        this.world.resize();
        if (!userDisabledP5Errors) p5.disableFriendlyErrors = false;
        let style = document.createElement("style");
        /* prevent callout to copy image, etc when tap to hold */ /* prevent webkit from resizing text to fit */ /* prevent copy paste, to allow, change 'none' to 'text' */ style.innerHTML = `canvas { 
			outline: none;
        ...
        if (idx > -1) {
            ...
            if (version < 16) pixelDensity(1);
            this.p5play.os.platform = "iOS";
            this.p5play.os.version = version;
        } else if (navigator.userAgentData !== undefined) this.p5play.os.platform = navigator.userAgentData.platform;
    };
    class Canvas {
        constructor(){
	 ...
	 */ this.background = function() {
        let args = arguments;
        let c;
        if (args.length == 1) c = p5.prototype.colorPal(args[0]);
	 ...
	 */ this.loadImage = function() {
        ...
        img = _loadImage.call(pInst, url, (_img)=>{
            _img.w = _img.width;
            _img.h = _img.height;
            for (let cb of _img.cbs)cb();
            for(let i = 1; i < _img.calls; i++)pInst._decrementPreload();
            // delete _img.calls;
            // delete _img.cbs;
            _img.cbs = [];
            pInst.p5play.images.onLoad(img);
	 ...
	 */ class InputDevice {
        ...
        init(inputs) {
            for (let inp of inputs)this[inp] = 0;
        }
        /**
    ...
    const __onmousedown = function(btn) {
        ...
        if (this.p5play.mouseSprites.length) {
            ...
            if (this.p5play.mouseSprite) {
                ...
                if (btn == "left") this.p5play.mouseSprite.mouse.draggable = false;
            }
    ...
    const __onmouseup = function(btn) {
        if (this.mouse[btn] >= this.mouse.holdThreshold) this.mouse[btn] = -3;
        else if (this.mouse[btn] > 1) this.mouse[btn] = -1;
        else this.mouse[btn] = -2;
        if (this.p5play.mouseSprite) {
            if (this.p5play.mouseSprite.mouse.hover > 1) {
                if (this.p5play.mouseSprite.mouse[btn] >= this.mouse.holdThreshold) this.p5play.mouseSprite.mouse[btn] = -3;
                else if (this.p5play.mouseSprite.mouse[btn] > 1) this.p5play.mouseSprite.mouse[btn] = -1;
                else this.p5play.mouseSprite.mouse[btn] = -2;
            } else {
                this.p5play.mouseSprite.mouse[btn] = 0;
    ...
    if (navigator.keyboard) {
        ...
        if (window == window.top) keyboard.getLayoutMap().then((keyboardLayoutMap)=>{
            const key = keyboardLayoutMap.get("KeyW");
            if (key != "w") this.p5play.standardizeKeyboard = true;
        });
        else this.p5play.standardizeKeyboard = true;
    } else // Firefox doesn't have navigator.keyboard
    // so just make it use key codes
    this.p5play.standardizeKeyboard = true;
    function getKeyFromCode(e) {
        let code = e.code;
        if (code.length == 4 && code.slice(0, 3) == "Key") return code[3].toLowerCase();
        return e.key;
    }
    ...
    this._onkeydown = function(e) {
        let key = e.key;
        if (this.p5play.standardizeKeyboard) key = getKeyFromCode(e);
        let keys = [
            key
        ];
        let k = simpleKeyControls[key];
        if (k) keys.push(k);
        for (let k1 of keys)if (!this.kb[k1] || this.kb[k1] < 0) this.kb[k1] = 1;
    ...
    this._onkeyup = function(e) {
        ...
        if (k) keys.push(k);
        for (let k1 of keys){
            if (this.kb[k1] >= this.kb.holdThreshold) this.kb[k1] = -3;
            else if (this.kb[k1] > 1) this.kb[k1] = -1;
            else this.kb[k1] = -2;
        }
    ...
    class Contros extends Array {
        constructor(){
            ...
            for (let m of methods)this[m] = (inp)=>{
            ...
            for (let prop of props){
                ...
                for (let axis of [
                    ...
                    get () {
                        if (_this1[0]) return _this1[0][prop][axis];
                        return 0;
                    }
                });
            }
...
p5.prototype.registerMethod("post", function p5playPostDraw() {
    ...
    if (this.allSprites.autoCull) this.allSprites.cull(10000);
    for (let btn of [
        "left",
        "center",
        "right"
        ...
        if (this.p5play.mouseSprite) {
            if (this.p5play.mouseSprite.mouse[btn] < 0) this.p5play.mouseSprite.mouse[btn] = 0;
        }
    }
    ...
    if (this.p5play.mouseTracking) {
        if (this.p5play.mouseSprite) {
            ...
            for (let btn1 of [
                ...
                "center",
                "right"
            ])val1 += this.p5play.mouseSprite.mouse[btn1];
            if (val1 == 0) this.p5play.mouseSprite = null;
        }
        ...
        if (this.mouse.pressing("left") || this.mouse.pressing("center") || this.mouse.pressing("right")) {
            ...
            if (!this.p5play.mouseSprite?.mouse.draggable) {
                ...
                for (let s of sprites)if (s == this.p5play.mouseSprite) {
                    ms = s;
                    break;
                }
            } else ms = this.p5play.mouseSprite;
        ...
        for (let s1 of sprites)s1.mouse.hover++;
        for (let s2 of this.p5play.mouseSprites)if ((!this.p5play.mouseSprite?.mouse.draggable || s2 != ms) && !sprites.includes(s2)) {
            s2.mouse.hover = -1;
            s2.mouse.left = 0;
            s2.mouse.center = 0;
            s2.mouse.right = 0;
            s2.mouse.draggable = false;
        }
...
//# sourceMappingURL=index.6a342110.js.map
